...
\section{Introduction}
This chapter discusses the technical implementation of the LexiFlow system. The development is divided into modular subsystems: User Management, Media Processing and AI, Interactive Learning, Assessment, DevOps and Deployment, Database Architecture, Security, and Supplementary Services. Each module is designed to fulfill the specific functional requirements outlined in the system design phase.

The frontend is implemented in \textbf{React 18 with TypeScript}, built using Vite, rather than the Vue 3 framework originally proposed during the design phase in Chapter 4. This change was made because the specific ecosystem of libraries required for the project --- the official Auth0 React SDK for authentication, TanStack Query for server-state caching and request de-duplication, and the available component libraries for building the interactive subtitle viewer --- had more mature and better-documented support in the React ecosystem at the time of implementation. The underlying Pipe and Filter architecture, use case coverage, and component responsibilities described in Chapter 4 remain unchanged; only the rendering framework used to build the user interface differs from what was originally planned.

The backend retains the dual-architecture described in the design phase: a Node.js (Express) server handles user-facing data management, business logic, and authentication enforcement, while a Python FastAPI server performs the computationally heavy AI media processing (speech recognition, translation, and pinyin generation).

\section{Module 1: User Management and Authentication}
The User Management module handles user identity, session access, and personalised data storage. User identity itself is delegated to Auth0 (see Module 7), while all application-specific user data is managed by the Node.js backend and stored in a PostgreSQL database via the Prisma ORM.

\subsection{Create Account (UC01) and Login (UC02)}
Account creation and login are not implemented as custom backend logic. Instead, both use cases are fulfilled entirely by \textbf{Auth0}, a third-party Identity-as-a-Service (IDaaS) provider, through its Universal Login flow (detailed in Module 7). When a user authenticates for the first time, the backend performs \textit{just-in-time (JIT) user provisioning}: it reads the unique subject identifier issued inside the verified access token and, if no matching record exists yet, automatically creates a corresponding user entry in the relational database through the \textbf{Prisma} Object-Relational Mapper (ORM). This guarantees that every authenticated identity from Auth0 has a corresponding relational record to which vocabulary lists, flashcard decks, and quiz history can be securely tied, without LexiFlow ever having to store or validate a password itself.

\subsection{Manage Profile (UC03)}
User profile information (display name, email, and picture) is sourced directly from the verified Auth0 identity token rather than from a dedicated, editable profile record in the database. A dedicated backend capability for storing and editing custom learning preferences (e.g., target learning language or default study parameters) has been scaffolded but is not yet populated with functionality; this is identified as outstanding work for a subsequent development phase.

One concrete account-management capability that \emph{is} fully implemented, however, is account deletion. A dedicated backend endpoint allows an authenticated user to permanently delete their account and all owned data in a single request. Most of the user's relational data (vocabulary lists, flashcard decks, quiz history) is removed automatically through cascading foreign-key deletion rules defined in the Prisma schema, so deleting the parent user row is sufficient to remove everything tied to it; data without a direct foreign-key relationship to the user (such as video-watch progress records) is deleted explicitly beforehand to avoid leaving orphaned rows.

\subsection{Learning Activity Dashboard}
To give users visibility into their own progress, the backend exposes a statistics endpoint that aggregates each learner's recent activity directly from the relational database: the number of new vocabulary words encountered, the number of flashcards reviewed, and the number of quizzes completed, each reported both for the current day and cumulatively across all time. Official HSK reference vocabulary lists are deliberately excluded from the "words seen" count, since adding the standard HSK list to one's profile is a one-time setup action rather than a daily learning activity. This summary is surfaced on the frontend dashboard to give learners a lightweight sense of momentum.

\section{Module 2: Media Processing and AI Engine}
This module represents the core pipeline of LexiFlow. The AI-driven media ingestion and linguistic generation work itself is performed by the FastAPI subsystem, but job orchestration is owned entirely by the Node.js backend: a request to process a video is first enqueued on \textbf{Agenda}, a MongoDB-backed job scheduling library running inside the Node.js process, against the same MongoDB Atlas cluster used for application data. The Agenda job handler then dispatches the work to a FastAPI worker over a simple REST contract, \texttt{POST /process} to start the job followed by repeated \texttt{GET /status/\{jobId\}} polling, and only the Node.js backend ever initiates these calls; the FastAPI worker never calls back into Node.js. This same contract is used identically against either FastAPI worker instance described in Module 5 (the local GPU server or the SaladPool failover), which Node.js selects between.

\subsection{Process YouTube URL (UC04) and Upload Local Media (UC05)}
Media ingestion is handled via an asynchronous processing pipeline within the FastAPI service.
\begin{itemize}
    \item \textbf{YouTube URL Processing (UC04):} The system uses \textbf{yt-dlp}, a widely-used open-source media downloader, to validate and retrieve video or audio content from YouTube (and other supported platforms, such as Bilibili) into a temporary workspace.
    \item \textbf{Local Media Upload (UC05):} Users can upload raw video/audio files directly.
\end{itemize}
Regardless of the source, incoming media is routed through \textbf{FFmpeg}, an open-source multimedia framework, which downsamples the audio into a clean 16kHz mono WAV PCM stream. This standardisation is critical for optimising the accuracy of the downstream speech recognition engines.

\subsection{Generate Subtitles (UC06)}
The subtitle generation process is a multi-step AI orchestration:
\begin{enumerate}
    \item \textbf{Voice Activity Detection (VAD):} \textbf{Silero VAD}, a pre-trained, open-source voice activity detection model, divides the audio stream into distinct speech windows, using multiprocessing to efficiently expand or fill detected gaps.
    \item \textbf{Speech-to-Text:} The processed audio chunks are transcribed using \textbf{faster-whisper}, a CTranslate2-optimised re-implementation of OpenAI's \textbf{Whisper} automatic speech recognition (ASR) model, which significantly reduces inference time and memory usage compared to the original Whisper implementation while producing equivalent transcriptions.
    \item \textbf{Linguistic Augmentation:} For Mandarin content, the system uses \textbf{PyPinyin}, an open-source Python library, to systematically extract tone-marked pinyin for each character. An external translation microservice, \textbf{LibreTranslate} (a self-hosted, open-source neural machine translation engine), is then queried via REST HTTP to provide context-aware English translations, compiling everything into a unified interactive subtitle format.
\end{enumerate}
Note that the translation engine used in the final implementation is LibreTranslate rather than the SeamlessM4T v2 model originally proposed in Chapters 2 and 4. This change was made because a self-hosted LibreTranslate instance is significantly lighter to run continuously (it does not require the large multilingual model weights or sustained GPU memory that SeamlessM4T v2 demands), making it a more practical fit for the project's current hosting budget while still meeting the required translation quality for Mandarin-to-English subtitle generation.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{./figs/fig_media_pipeline.pdf}
    \caption{Media Processing and Subtitle Generation Pipeline}\label{fig:media_pipeline}
\end{figure}

\section{Module 3: Interactive Learning Environment}
This module serves as the primary user workspace, bridging the media content with active language acquisition tools in the frontend.

\subsection{Find Translated Content (UC07)}
Users can browse and locate previously processed and translated media through a library interface that lists, filters, sorts, and searches videos by title, date, and duration. The backend retrieves paginated video metadata and user progress states, rendering a gallery of accessible learning materials. Each video also carries a visibility flag, allowing the library to distinguish content the user has personally processed from material made public for other learners.

\subsection{Search Word Definition (UC08)}
The video playback interface renders a bilingual subtitle engine. Individual Chinese characters are tagged according to their HSK difficulty level (e.g., HSK 1, HSK 2). Hovering over or tapping a word triggers a dictionary lookup powered by \textbf{CC-CEDICT}, an open-source, community-maintained Chinese-English dictionary dataset, displaying the definition and pinyin dynamically without interrupting video playback. The same view also exposes full-text search across previously generated subtitles (Chinese text, pinyin, and English translation).

\subsection{Save Vocabulary (UC09) and Bookmark Video (UC10)}
Vocabulary acquisition is deeply integrated with video playback.
\begin{itemize}
    \item \textbf{Save Vocabulary (UC09):} Users can automatically or manually save words encountered while watching a video into a personal vocabulary list. Each saved entry is validated at the API boundary using \textbf{Zod}, a TypeScript-first schema validation library, to ensure that saved items always contain the simplified character, pinyin, meaning, and contextual metadata.
    \item \textbf{Context Bookmarking (UC10):} When a word is saved, the system captures the source video, the exact timestamp, the surrounding context sentence, and its translation, effectively bookmarking the video segment so users can revisit the exact origin of the vocabulary item.
\end{itemize}

\subsection{Export Vocabulary List (UC13)}
LexiFlow allows learners to take their saved vocabulary outside the platform, supporting export in three formats: a plain CSV file, an Anki-compatible flashcard import file, and a formatted PDF document. PDF generation uses \textbf{PDFKit}, an open-source PDF generation library, together with the embedded open-source \textbf{Noto Sans SC} font, which is required to correctly render Simplified Chinese glyphs inside the generated PDF. Conversely, learners can import an existing CSV or plain-text vocabulary list, with entries parsed and de-duplicated before being inserted as a new list.

\section{Module 4: Review and Assessment}
To ensure the long-term retention of saved vocabulary, LexiFlow features a comprehensive spaced repetition system and an AI-driven quiz generator. These systems transition learners from passive media consumption to active recall.

\subsection{Generate and Review Flashcards (UC13)}
LexiFlow utilises a Spaced Repetition System (SRS) for optimal vocabulary retention. The scheduling logic is a direct implementation of the \textbf{FSRS (Free Spaced Repetition Scheduler) algorithm}, an open-source spaced repetition model originally developed for the Anki flashcard ecosystem (\url{https://github.com/open-spaced-repetition/fsrs4anki}), which has been shown to outperform traditional algorithms such as SM-2 used by legacy spaced-repetition tools. LexiFlow implements the FSRS formulas and its published default weight parameters directly in the backend business logic, rather than depending on a third-party scheduling service.

The FSRS algorithm calculates three primary memory states for each vocabulary item: Retrievability (the probability of recalling the item), Stability (the memory's resistance to forgetting), and Difficulty (the inherent complexity of the word). Users interact with flip-style flashcards and grade their recall using a four-button matrix:
\begin{itemize}
    \item \textbf{1 (Again):} User failed to recall. The system resets the stability and schedules an immediate review.
    \item \textbf{2 (Hard):} User recalled with significant difficulty. The interval is slightly increased.
    \item \textbf{3 (Good):} User recalled standardly. The stability coefficient increases, pushing the due date further out.
    \item \textbf{4 (Easy):} User recalled effortlessly. The interval is maximally expanded.
\end{itemize}
Whenever a grade is submitted, the backend recomputes the FSRS memory variables, persists them to the relational database, and schedules the exact timestamp for the next review. Figure~\ref{fig:fsrs_growth} illustrates how the scheduled interval until the next review grows for a single vocabulary item across consecutive "Good" reviews: early reviews are scheduled only a day or two apart, while well-retained items are pushed weeks or months into the future, concentrating review effort on the items a learner is closest to forgetting.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.75\textwidth]{./figs/fig_fsrs_growth.pdf}
    \caption{Example FSRS Scheduled-Interval Growth for One Vocabulary Item}\label{fig:fsrs_growth}
\end{figure}

\subsection{Generate Quizzes (UC14)}
Automated quiz generation is handled by the FastAPI AI engine, which dynamically synthesises assessment materials from the user's saved vocabulary lists. The system explicitly supports four question types: multiple choice, fill-in-the-blank, short answer, and true/false.

\subsubsection{Question Types and Evaluation}
\begin{itemize}
    \item \textbf{Fill-in-the-Blank:} The system uses regex-driven pattern matching to extract the contextual sentences saved from the media player. It intercepts the target vocabulary word and replaces it with underscore indicators (e.g., ``\_\_\_\_''), requiring the user to type the exact character or pinyin.
    \item \textbf{Short Answer:} Instead of relying on rigid string matching, LexiFlow employs an open-ended semantic evaluator built on the pre-trained \textbf{\texttt{paraphrase-multilingual-MiniLM-L12-v2}} model, distributed through the open-source \textbf{Sentence-Transformers} library (\url{https://github.com/UKPLab/sentence-transformers}). This model was chosen specifically because it is multilingual and produces meaningful semantic embeddings for both Mandarin and English text, which is required since LexiFlow's vocabulary and answers span both languages. The system generates embedding vectors for both the user's input and the reference definition, then computes their Cosine Similarity. The answers are rated across the following boundaries:
    \begin{itemize}
        \item \textbf{$>$ 0.90:} Exact match or perfect synonym.
        \item \textbf{$>$ 0.80:} Acceptable meaning.
        \item \textbf{$>$ 0.70:} Partial credit (contains related semantic value).
        \item \textbf{$<$ 0.70:} Incorrect response.
    \end{itemize}
\end{itemize}

\subsubsection{Distractor Generation}
For multiple-choice and true/false questions, the system must generate plausible but incorrect answers, known as distractors. High-quality distractors are crucial to prevent the user from guessing the answer by basic elimination. The distractor algorithm fetches vocabulary from the same HSK tier or lexical category as the target word. To guarantee the distractors are not accidentally valid synonyms, the system evaluates them against the target word using the same \texttt{paraphrase-multilingual-MiniLM-L12-v2} Sentence-Transformers embedding model described above. If the similarity score is too high (e.g., $>$ 0.70), the distractor is rejected and a new, more distinct word is selected.

\section{Module 5: DevOps and Deployment Pipeline}
The production environment of LexiFlow is split across several distinct hosting environments, each chosen to match the resource profile of the component it serves: a static-asset CDN for the frontend, a low-cost always-on cloud VM for the relational backend, a managed cloud database for the legacy/job-queue data, one or more GPU-capable hosts for the AI media engine, and a managed inference cloud for the LLM-based correction feature (Module 8). Crucially, the Node.js backend is the sole orchestrator of this topology: the frontend and every other component communicate only through it, never directly with each other. Figure~\ref{fig:deployment_topology} shows how these environments are connected, including the automated CI/CD path, the Agenda-backed job queue, and the GPU failover path described in the following subsections.
...
The React/TypeScript frontend application is hosted on \textbf{Vercel}. Vercel acts as an Edge CDN (Content Delivery Network), ensuring that static assets and compiled JavaScript chunks are served to users with minimal latency globally. The CI/CD pipeline is integrated directly into the repository: upon every push or pull request to the main branch, Vercel automatically triggers a build instance (\texttt{npm run build}), executes frontend linters, and immediately deploys the updated preview or production build.

\subsection{Backend and Database Hosting}
The Node.js business logic backend and the PostgreSQL database are hosted on a \textbf{Tencent Cloud Lighthouse} server. To maintain environment consistency between development and production, the backend is fully containerised using \textbf{Docker}.
\begin{itemize}
    \item \textbf{Automated CI/CD:} A \textbf{GitHub Actions} workflow is triggered on every push to the main branch that affects the backend codebase. The workflow builds a new Docker image for the backend, pushes it to the \textbf{GitHub Container Registry (GHCR)}, then opens an SSH connection to the Tencent Lighthouse server to pull the new image and restart the corresponding service via Docker Compose, pruning old, unused images afterwards. This means the Node.js backend is automatically redeployed on every merge to main without manual intervention.
    \item \textbf{Database Management:} PostgreSQL runs directly on the Tencent Lighthouse host, reachable by the containerised backend over the host network. Prisma ORM migrations are executed automatically during the container startup sequence, ensuring the PostgreSQL schema remains synchronised with the Node.js application models.
\end{itemize}

\subsection{AI Media Engine Hosting}
Because the FastAPI backend handles computationally expensive AI tasks --- such as Whisper speech-to-text, video transcoding via FFmpeg, and embedding-based semantic evaluation --- it requires sustained GPU access that would be costly to rent continuously from a cloud GPU provider. For this reason, the primary deployment of the FastAPI AI media engine is a \textbf{local, on-premises GPU server} maintained by the developer, which is used throughout both development and the current production testing phase. This avoids the recurring cost of cloud GPU rental for a service that, at the present user scale, does not yet require elastic, multi-node scaling. The Node.js backend is the only caller of this service: as described in Module 2, it dispatches each job with \texttt{POST /process} and then polls \texttt{GET /status/\{jobId\}} until completion, a request/response relationship rather than a callback from FastAPI back to Node.js.
\begin{itemize}
    \item \textbf{Failover:} \textbf{SaladPool} (Salad Cloud), a distributed network of rented consumer GPUs, is retained as a second FastAPI worker target, reachable over the identical \texttt{/process}/\texttt{/status} contract as the local server. The Node.js backend monitors the depth of the Agenda job queue and automatically routes new jobs to SaladPool, scaling the rented worker up or down as needed, whenever the local server is unreachable, offline for maintenance, or unable to keep up with the current queue depth, so that media processing requests keep being served at the cost of per-use cloud GPU rental rather than the fixed cost of local hardware.
\end{itemize}

\section{Module 6: Database Architecture and Polyglot Persistence}
To optimally manage the diverse data structures within LexiFlow, the system adopts a polyglot persistence architecture. This approach utilizes two distinct database technologies—PostgreSQL and MongoDB—each tailored to specific workload requirements and data models.

\subsection{Relational Data Management with PostgreSQL}
PostgreSQL serves as the primary relational database for the application's core business logic. It is managed via the Prisma Object-Relational Mapper (ORM) within the Node.js backend. PostgreSQL is strictly utilized for highly structured entities that require ACID (Atomicity, Consistency, Isolation, Durability) compliance and rigid referential integrity. 
\begin{itemize}
    \item \textbf{Data Handled:} User profiles, vocabulary lists, saved words, flashcard decks, FSRS spaced repetition algorithm states, user quiz scores, and the AI-generated segment corrections described in Module 8.
    \item \textbf{Implementation:} The Prisma schema enforces strict foreign key constraints and cascading deletions (e.g., deleting a user automatically removes their custom vocabulary lists). This guarantees relational data consistency across the learning and assessment modules.
\end{itemize}

\subsection{Document Storage and Queue Management with MongoDB}
Conversely, MongoDB is implemented as a NoSQL document database to handle the unstructured and semi-structured data generated by the FastAPI media processing pipeline. The MongoDB instance is hosted on \textbf{MongoDB Atlas}, a managed cloud database service, rather than as a self-hosted container; this removes the operational burden of managing replication and backups for a secondary database. Its flexible, document-oriented model is ideal for the high-throughput write operations required by the AI engine.
\begin{itemize}
    \item \textbf{Subtitle and Job Storage:} The outputs from the Whisper AI model contain complex, hierarchical JSON data, including raw text, exact timestamps, pinyin mappings, and translation layers. MongoDB stores these rich subtitle documents, the underlying processing job records, and crowd-sourced segment reports (Module 8) without requiring rigid schema migrations whenever the AI output format or metadata evolves.
    \item \textbf{Queue System:} The asynchronous nature of media processing (downloading, audio extraction, transcription, and translation) requires a robust task queue. This is implemented with \textbf{Agenda}, a MongoDB-backed job scheduling library that runs inside the Node.js backend (Module 2) and stores its queue collection in the same MongoDB Atlas cluster used for application data. Agenda tracks the real-time status of each asynchronous job, recording the progress reported by whichever FastAPI worker (Module 5) is currently processing it, and allows the system to update the frontend on video processing progress without locking or overloading the primary PostgreSQL database.
    \item \textbf{Role relative to PostgreSQL:} As the system has matured, newer features (vocabulary management, flashcards, and quizzes) have been implemented directly on PostgreSQL rather than MongoDB. MongoDB's current role is therefore narrower than originally scoped in Chapter 4 --- it is retained specifically for video-processing job state, raw subtitle documents, and segment reports, while PostgreSQL serves as the system of record for all user-owned learning data.
\end{itemize}

\section{Module 7: Security and Authentication}
Ensuring secure access to user data and backend APIs is a critical requirement. LexiFlow offloads the complexity of credential management by integrating Auth0, a centralized Identity-as-a-Service (IDaaS) provider, to manage all authentication and authorization flows.

\subsection{Auth0 Universal Login and Authentication Flow}
The application utilizes the industry-standard OAuth 2.0 Authorization Code flow, via the official Auth0 React SDK. When a user attempts to log in or create an account (UC01 and UC02), the React frontend redirects them to Auth0's Universal Login interface. This provides a secure, centralised authentication page that supports both traditional email/password login and external Single Sign-On (SSO) providers. By handling authentication on Auth0's isolated servers, LexiFlow abstracts the risks of password storage and protects the system against common vulnerabilities such as brute-force attacks. A demo mode is also supported in the frontend: if Auth0 environment variables are not configured (e.g. for local UI development), the application renders without requiring authentication, though authenticated backend calls will not succeed in that mode.

\subsection{Backend API Authorization via JWT}
Once the user is successfully authenticated, Auth0 returns a cryptographically signed JSON Web Token (JWT). This token acts as the user's secure digital identity payload for the application's ecosystem.
\begin{itemize}
    \item \textbf{Frontend Handling:} The React application retrieves the access token through the Auth0 React SDK and attaches it as a \texttt{Bearer} token within the \texttt{Authorization} header of all HTTP requests made to the Node.js backend.
    \item \textbf{Middleware Verification:} The Node.js server implements an authentication middleware built on \textbf{\texttt{express-jwt}} and \textbf{\texttt{jwks-rsa}}, two open-source libraries for verifying JWTs against a remote JSON Web Key Set (JWKS). Before any protected route (such as fetching a custom vocabulary list or saving a new word) is executed, the middleware extracts the JWT and validates it against the Auth0 tenant's public JWKS, verifying the token's RS256 signature, expiration, and audience claims. If the token is valid, the user's Auth0 subject ID is extracted and passed to the relevant module (triggering the just-in-time user provisioning described in Module 1 on first use); if invalid or expired, the API immediately rejects the request with a \texttt{401 Unauthorized} status. This middleware is applied uniformly across every module described in this chapter --- vocabulary, videos, flashcards, quizzes, TTS, translation, and reports --- so that all user-owned data remains isolated per authenticated identity.
\end{itemize}

\section{Module 8: Supplementary Services}
Beyond the core four modules, LexiFlow implements three supplementary services that support the learning experience: text-to-speech playback, on-demand translation, and community-driven quality reporting.

\subsection{Text-to-Speech (TTS)}
LexiFlow exposes an endpoint that converts Mandarin text into spoken audio. Two interchangeable TTS backends are supported, selected through configuration:
\begin{itemize}
    \item \textbf{Edge TTS (default):} An open-source wrapper around Microsoft Edge's cloud-based text-to-speech engine, which requires no local GPU resources and is used as the default backend for low-cost, low-latency synthesis.
    \item \textbf{CosyVoice (optional):} \textbf{CosyVoice}, an open-source, GPU-accelerated speech synthesis model (the Fun-CosyVoice3-0.5B variant), self-hosted as its own containerised service and enabled selectively when higher-quality, locally controlled speech synthesis is required.
\end{itemize}
To avoid redundant computation, generated audio is cached to disk, keyed by a hash of the requested voice and text, and served back directly on subsequent identical requests.

\subsection{On-Demand Translation}
LexiFlow also allows the frontend to request an ad-hoc translation of arbitrary Mandarin or English text (e.g., outside the context of a processed video), independent of the main subtitle generation pipeline. This is backed by the same self-hosted \textbf{LibreTranslate} engine used in the media processing pipeline (Module 2). Results are cached in memory to avoid repeatedly translating identical strings.

\subsection{Crowd-Reporting of Translation Errors (UC11)}
To maintain content quality, users can flag a specific subtitle segment as incorrect (categorised as a translation error, a pinyin error, a transcription error, or other). A segment is automatically marked as flagged for review once it accumulates at least three distinct user reports \emph{and} at least 25\% of the users who viewed that segment have reported it, preventing a single user from flagging content unilaterally.

\subsection{AI-Assisted Segment Correction}
Rather than requiring an administrator to manually re-transcribe or re-translate every flagged segment, LexiFlow now resolves a report immediately by delegating the correction to a Large Language Model (LLM). The integration is built behind a small provider-agnostic interface so the underlying LLM vendor can be swapped through configuration alone; the active implementation calls \textbf{Meta's Llama 3.3 70B} model through \textbf{Groq}, an LLM inference cloud chosen specifically for its very low response latency, via Groq's OpenAI-compatible Chat Completions API.

When a report is filed, the system sends the model the reported segment together with the three segments immediately before and after it (for disambiguating context only), along with an instruction specific to the reported error category (translation, pinyin, transcription, or other). The model is constrained to return a single, strict JSON object containing the corrected Chinese text, English translation, and per-character pinyin/HSK metadata, with its response format enforced through Groq's JSON-mode decoding. The original timestamps are never trusted to the model's output; they are always re-applied server-side from the original segment, so a hallucinated value can never desynchronise the subtitle track from the underlying audio. Figure~\ref{fig:ai_correction_flow} summarises the full correction flow from the initial viewer report to the corrected subtitle being served back to all viewers.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{./figs/fig_ai_correction_flow.pdf}
    \caption{AI-Assisted Segment Correction Flow}\label{fig:ai_correction_flow}
\end{figure}

Crucially, the correction is never written back into the original transcription record in MongoDB. Instead, it is stored as a separate, addressable patch in a dedicated relational table (linked to the specific video and segment index), which is transparently merged over the original segment data whenever that video's subtitles are subsequently read. This design keeps the original AI-generated transcription fully intact and auditable while still letting corrected content reach every viewer immediately, and makes a correction trivially reversible if it later proves to be wrong. Each correction is also logged with the before/after segment pair and the identity of the model that produced it, and the reporting user is asked to rate the quality of the correction; this creates a growing labelled dataset that could be used in future work to evaluate or fine-tune correction quality across models.

\section{Module 9: Onboarding and User Experience}
Beyond the core learning features, several supporting capabilities were implemented to make the system more approachable for first-time visitors and more usable during day-to-day study sessions.

\subsection{Unauthenticated Landing Preview}
To let a visitor evaluate LexiFlow before creating an account, the public landing page embeds a short, pre-processed video preview showing the bilingual subtitle experience in action. The preview clip's data (subtitles, pinyin, and translation) is intentionally stored as a static file within the backend source tree rather than fetched live from the main video-processing data store, so that the marketing preview keeps working even if the underlying video record is later modified or removed. Only a fixed, server-enforced time window of the clip is ever served to unauthenticated visitors, regardless of what the frontend requests, so the full experience remains a logged-in benefit.

\subsection{Persistent Background Job Tracking}
Processing a submitted video is not instantaneous, so LexiFlow tracks the active processing job persistently rather than tying it to a single page. The job's state (queue position, progress percentage, and estimated time remaining) is held in the browser's local storage and continuously polled in the background, independent of which page the user is currently viewing. This allows a small floating status indicator to remain visible everywhere in the application while a video is processing, and lets the user navigate away and return later without losing track of their submission or being shown a duplicate completion notification.

\subsection{Guided Product Tour and Theming}
For new users, an interactive, step-by-step guided tour highlights the main areas of the interface (the library, the watch page, vocabulary, flashcards, and quizzes) the first time they sign in. Independently of the tour, the frontend also supports both light and dark colour themes, with the user's preference persisted across sessions, to accommodate different study environments and personal preference.

\section{Module 10: Verification and Validation}
With the modules above implemented, the system was verified at two complementary levels. First, an automated end-to-end test suite was built with Playwright and run directly against the live test deployment, covering UC01 through UC14 as well as the supporting features described in Module 9. This run produced 25 passes, 4 failures, and 1 documented skip out of 30 test cases; the 4 failures were traced to two genuine, reproducible defects in the system, an onboarding tour that cannot be dismissed and a missing single-page-application fallback route on direct navigation, rather than to flaws in the test scripts themselves. The full methodology, the per-test-case results table, and both defects' root-cause analyses (with screenshot evidence) are presented in Appendix~\ref{app:test-execution-report}.

Second, a round of User Acceptance Testing was carried out with four participants representing the distinct user groups identified in Chapter 4: a learner who had previously struggled with a Mandarin module, a self-directed learner with a domain-specific (robotics) vocabulary need, a native-speaker reviewer assessing real-world transcription and translation accuracy, and the original instructor stakeholder. Their feedback independently confirmed the value of the tap-to-look-up, vocabulary-saving, and quiz-generation workflow, while also surfacing two honest findings for future work: an accuracy gap against commercial-grade captioning (which the system's existing crowd-correction feature is designed to narrow over time), and the absence of any classroom-management capability (class creation, assignment of videos, and a lecturer-facing progress view), which was never part of the functional requirements scoped in Chapter 4. The full participant profiles, scenarios, verbatim reflections, and acceptance-criteria analysis are presented in Appendix~\ref{app:uat-report}.

\chapter{CONCLUSION}
\section{Introduction}
This chapter gives an overview of the LexiFlow project's development, outcomes, and the extent to which the project's goals were achieved. It reflects on the findings from both rounds of system evaluation, namely automated end-to-end test execution and User Acceptance Testing (UAT) with real participants, highlights the project's significant accomplishments, and identifies areas for future improvement. This review outlines the overall effectiveness of the LexiFlow platform in addressing the concerns of educators and learners, alongside its future potential to improve and scale. The LexiFlow project has succeeded in developing a fully implemented, deployed, and independently tested language learning system that meets the core expectations set by its primary user group, the self-directed Mandarin learner, while also surfacing an honest, evidence-based picture of where the system still falls short, namely a measurable accuracy gap against commercial-grade captioning and the absence of classroom-management capabilities for instructors. The architecture and technology decisions made over the course of the project, including the move from the originally proposed Vue frontend to React with TypeScript and Vite, the migration of subtitle/translation generation onto a self-hosted faster-whisper and LibreTranslate pipeline, and the adoption of a multi-cloud deployment topology (Vercel, Tencent Cloud Lighthouse, MongoDB Atlas, and a local GPU server backed by SaladPool), now form a solid, production-deployed base for any further enhancements. Through continued improvement, particularly the crowd-correction and AI-assisted segment correction features already built into the system, LexiFlow can keep closing the accuracy gap identified during UAT and stand out as a reliable, user-friendly Mandarin learning solution for university and self-directed settings alike.
...
\begin{enumerate}[label= (\alph*)]
    \item \textbf{To identify and gather requirements from primary stakeholders, including language learners and educators:} This objective was successfully accomplished through a series of interviews with stakeholders, including a UTM Language Academy Mandarin instructor. These interactions provided valuable insights into the language learning needs and challenges within the UTM community, and formed the project's functional and non-functional specifications (Chapter 4). The subsequent UAT round (Appendix~\ref{app:uat-report}) revisited this same stakeholder group and confirmed that the elicited requirements were largely well-understood and addressed, while also surfacing one requirement gap, classroom-management capability, that had not been explicitly captured during the original requirements-gathering phase. This objective is considered successfully achieved, with the system requirements effectively collected, analysed, and validated against real stakeholder feedback post-implementation.
    \item \textbf{To design a web-based platform with a user interface and a database that provides synchronized, time-stamped bilingual subtitles:} The LexiFlow application was designed using a Pipe and Filter architecture model for its media-processing pipeline, with a relational PostgreSQL database as the system of record and MongoDB Atlas retained for jobs, raw subtitle payloads, and reports. The originally proposed Vue frontend was implemented instead with React, TypeScript, and Vite for stronger ecosystem and tooling fit, a deviation explained and justified in Chapter 5. The implemented system produces synchronized, timestamped, pinyin-annotated bilingual subtitles exactly as designed, and this has been independently confirmed both by automated tests (TC-14 in Appendix~\ref{app:test-execution-report}) and by every UAT participant who interacted with the watch page. This objective is considered successfully achieved.
    \item \textbf{To develop a web platform based on stakeholder requirements, adding features such as audio/video processing, transcription, translation, and pinyin generation:} The full platform was implemented as detailed in Chapter 5, including the audio processing pipeline (yt-dlp, FFmpeg, Silero VAD, faster-whisper, PyPinyin, and LibreTranslate), vocabulary saving with FSRS-based spaced-repetition flashcards, AI-generated quizzes, crowd-reporting with Groq/Llama-3.3-70B-assisted segment correction, a Learning Activity Dashboard, an unauthenticated landing preview, a guided product tour, and light/dark theming. All fourteen use cases (UC01-UC14) identified in Chapter 4 were implemented and deployed to a live, publicly reachable test environment (Section 5, Module 10). This objective is considered successfully achieved, with development extending beyond the originally scoped feature set to include several of the supporting capabilities described in Module 9.
    \item \textbf{To test and validate the platform using testing methodologies and domain expert's feedback to ensure accuracy in transcription, translation, and subtitle synchronization:} This objective, previously deferred to a later phase, has now been completed. An automated Playwright end-to-end test suite was executed against the live deployment, achieving 25 passes, 4 failures, and 1 documented skip out of 30 test cases (Appendix~\ref{app:test-execution-report}); the 4 failures were root-caused to two genuine, reproducible defects (an undismissable onboarding tour, and a missing single-page-application fallback route) rather than to test-script issues. Domain-expert validation was obtained directly through UAT with a native Mandarin speaker, who judged transcription and translation accuracy as acceptable for a learning tool but below the bar of mature commercial captioning services, and through the original instructor stakeholder, who remained satisfied with accuracy but identified the classroom-management gap discussed above (Appendix~\ref{app:uat-report}). This objective is considered successfully achieved, with both the strengths and the limitations of the implemented system now documented with concrete, traceable evidence rather than left as an open task for a later phase.
\end{enumerate}
