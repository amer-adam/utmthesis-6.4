...
\subsection{Non-Functional Requirement Verification}\label{sec:nfr_verification}To check NFR-001 --- content has to process within 10 minutes per hour of source-content duration --- 16 real videos, ranging from 11 seconds to 55 minutes, went through the production media-processing pipeline on CUDA-accelerated infrastructure, with end-to-end processing time recorded for each. Table~\ref{tab:nfr_performance} lists the results sorted by content length: the ratio of processing time to content duration, the implied processing minutes per hour, and whether each run hit the target. \begin{table}[H]
    \centering
    \footnotesize
    ...
    \end{flushleft}
\end{table} Eleven of sixteen runs (68.8\%) met the target, and content nine minutes or longer met it in every run but one. The five misses were, bar one exception, all short clips under 11 minutes --- each processing job carries a largely fixed startup cost (model/API connection setup, segmentation initialisation) that's independent of content length, so that overhead weighs proportionally heavier, and the minutes-per-hour ratio comes out worse, for short inputs. The exception, V9 (18 minutes 52 seconds), ran considerably longer than its neighbours in the dataset; that's treated as a genuine outlier rather than swept aside, and its cause (maybe a spell of elevated load on an externally hosted API) wasn't isolated this round. On the whole, the pipeline reliably hits its performance target for the long-form authentic content LexiFlow is actually built around, while short-clip processing --- an edge case outside the platform's main use case --- stays an area worth optimising later, say by amortising or cutting down that fixed per-job startup overhead.
