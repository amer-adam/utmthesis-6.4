...
\begin{abstract}
    LexiFlow addresses the critical challenge of making authentic Mandarin content accessible and comprehensible for language learners. Traditional methods for creating learning materials are labor-intensive, requiring manual transcription, translation, and annotation, which limits the availability of engaging resources. Existing platforms often depend on pre-existing subtitles and lack support for Mandarin-specific features such as pinyin and sentence-level segmentation. LexiFlow is a web-based platform that automates the processing of Mandarin audio and video by leveraging advanced voice activity detection, automatic speech recognition, neural machine translation, and pinyin generation. The system segments content into linguistically meaningful units, generates synchronized bilingual subtitles with pinyin, and stores processed materials in a searchable database for instant reuse. This approach reduces the workload for educators, expands the library of authentic content, and provides learners with accurate, time-aligned transcriptions and translations. This system was developed using a hybrid software development methodology of the Waterfall model with Agile Scrum. LexiFlow’s modular, pipe-and filter architecture ensures scalability and maintainability, while its user-driven content model fosters personalized learning. It is hoped that the development of LexiFlow can demonstrate the feasibility and effectiveness of integrating state-of-the-art language technologies to enhance Mandarin language acquisition through real-world content.
\end{abstract}

\begin{abstrak}
    LexiFlow menangani cabaran kritikal untuk menjadikan kandungan Mandarin tulen boleh diakses dan difahami oleh pelajar bahasa. Kaedah tradisional untuk menghasilkan bahan pembelajaran memerlukan tenaga kerja yang tinggi, memerlukan transkripsi manual, terjemahan dan anotasi, yang mengehadkan ketersediaan sumber yang menarik. Platform sedia ada selalunya bergantung pada sari kata sedia ada dan kekurangan sokongan untuk ciri khusus Mandarin seperti pinyin dan pembahagian peringkat ayat. LexiFlow ialah platform berasaskan web yang mengautomasikan pemprosesan audio dan video Mandarin dengan memanfaatkan pengesanan aktiviti suara lanjutan, pengecaman pertuturan automatik, terjemahan berasaskan rangkaian neural dan penjanaan pinyin. Sistem membahagikan kandungan ke dalam unit yang bermakna dari segi bahasa, menjana sari kata dwibahasa yang diselaraskan dengan pinyin dan menyimpan bahan yang diproses dalam pangkalan data yang boleh dicari untuk digunakan semula segera. Pendekatan ini mengurangkan beban kerja untuk pendidik, mengembangkan perpustakaan kandungan tulen dan menyediakan transkripsi dan terjemahan yang tepat dan selaras masa kepada pelajar. Sistem ini dibangunkan menggunakan metodologi pembangunan perisian hibrid model Waterfall dengan Agile Scrum. Seni bina modular, pipe-and-filter yang digunakan oleh LexiFlow memastikan kebolehskalaan dan kebolehselenggaraan, manakala model kandungan dipacu penggunanya memupuk pembelajaran yang diperibadikan. Adalah diharapkan pembangunan LexiFlow dapat menunjukkan kebolehlaksanaan dan keberkesanan penyepaduan teknologi bahasa terkini untuk meningkatkan penguasaan bahasa Mandarin melalui kandungan dunia sebenar.
\end{abstrak}
...
Mandarin is not only a language but a key to future opportunities, as it is spoken by more than one billion people and is gaining significant importance in business, technology, and culture around the world. The growing importance of China as an economic power, a leader in manufacturing and artificial intelligence, and its increasing influence in the cultural arena opens doors to career opportunities for Mandarin speakers. However, many learners perceive the language as too complicated , both in its tonal pronunciation and its complex character system , making it seem impossible to master.

The studies in language acquisition have indicated that exposure to authentic content is the most effective way to learn a language~\citep{krashen1985,Mayer_2009}. Here authentic means that the material is genuine (real) and originally written for native speakers (books, movies, news articles, podcasts, conversations, etc.), not simplified or made-up for language learners. If students are interested in the materials, they will be more motivated and will retain more. This natural learning approach mirrors how children acquire their first language: through immersion, not memorization~\citep{cychosz2021, gowenlock2024}.

Many students encounter difficulties when learning through traditional methods. Textbooks and classrooms tend to focus more on grammar rules than on exposure to natural language. While some translation applications now exist, they struggle with the specific demands of Mandarin. The character system poses a significant challenge because written characters bear no resemblance to their spoken sounds. Pinyin is available as a pronunciation guide but is seldom encountered outside the classroom.

LexiFlow\footnote{The live application is accessible at \url{https://lexiflow.amerai.top}, and the source code is available on GitHub at \url{https://github.com/amer-adam/lexiflow}.} addresses these challenges through a web platform that integrates sentence segmentation and machine translation models. The system receives audio, segments it into sentences, and processes each one separately. This will help to increase accuracy and reduce the computing resources required. The system then maintains a repository of previously processed material to avoid reprocessing and to surface content tailored to students' interests and abilities.
\section{Problem Background}

The lecturer of language at UTM has noticed a problem with the development of language among the students at UTM. A lot of students may be able to learn the written grammar rules and new vocabulary for their written tests but not be able to apply them in actual conversations in Mandarin. This is a gap between student learning and communication skills that may restrict student learning. Authentic content presents students with words, natural speech patterns and culture that they would not find in textbooks, particularly if authentic content connects with students' interests, making the language more engaging.

The current method of producing Mandarin learning content is still time-consuming. Lecturers start by gathering authentic audio or video sources, often spending considerable time finding appropriate content. After selecting content, teachers manually transcribe the spoken Mandarin and add pinyin annotations and translations. This process can take as long as 8-10 hours for just a few minutes of content and can put a significant strain on teachers.

These challenges are not met by existing solutions. Popular platforms such as LanguageReactor rely heavily on the availability of YouTube subtitles, which are far harder to find for Mandarin content than for English. As for general transcription services, for example the ones at Transcribe.com, they transcribe material without the time-stamped subtitle format that students find useful in mapping spoken sounds to written text, and they do not support pinyin, which is essential for Mandarin learners to learn how to pronounce the text.

Students need a system that effectively processes authentic Mandarin content and presents both its spoken form and written representation. The solution should include accurate transcriptions and timings, pinyin pronunciation guides, and quality translations that link naturally spoken Mandarin with its written form.
\section{Project Aim}
The aim of this project is to create a web platform called LexiFlow for the processing, transcription, and translation of authentic Mandarin content using advanced sentence-level segmentation and translation models, in order to help students understand authentic Mandarin dialogues in real-world contexts.
...
\begin{enumerate}[label= (\alph*)]
    ...
    \item The proposed platform will generate transcriptions using automatic speech recognition (ASR) and translations via a Neural Machine Translation (NMT) model.
    ...
    \item The proposed platform will provide a web interface for uploading content and customizing subtitle outputs.
\end{enumerate}
...
The thesis consists of six well-rounded chapters. Chapter 1 presents the project and provides background, purpose, objectives, scope, importance and structure of the report. Chapter 2 presents an extensive literature review on the current systems and technologies pertaining to speech recognition, translation models, and user interfaces, and finally a comparison with the available market solutions.
...
Chapter 5, Implementation and Testing, details how the architecture was realised in practice, the core features that were built, the DevOps and deployment process, and the verification and validation activities used to test the system. In the last chapter (Chapter 6), the findings from the project are summarized, the project outcomes discussed and recommendations made for improvements to further develop and extend the LexiFlow system. This structure ensures a logical progression from conceptualization through implementation and evaluation, with detailed discussion of all project aspects.
\chapter{LITERATURE REVIEW}
\section{Introduction}
A review of the relevant literature and technologies has been conducted to examine existing work pertaining to the development of the LexiFlow system. It begins with a review of theories and models of second language acquisition that are relevant to the design of any L2 learning material. It then underscores the problems associated with symbolic language and discusses the existing methods and systems in the area of language acquisition. The purpose of this literature review is to give a foundation of theory and to highlight gaps that LexiFlow is looking to fill.
...
Consider, for example, an HSK 2 student who can handle simple, everyday conversations. Based on the "i+1" principle by Krashen, such a learner acquires language most effectively when exposed to content at approximately HSK 3 level , introducing new vocabulary and grammatical patterns in manageable quantities while still containing familiar elements.
...
\citet{jones2002} showed that students' vocabulary retention improved by 42\% when using both visual cues and written text, as opposed to learning from the written text alone. In character-based languages,~\citet{shen2010} concluded that a multimedia presentation linking spoken words, written characters, and visual representations was 35\% more effective for character recognition than traditional methods.

In addition,~\cite{li2022} demonstrated that Mandarin learners achieved higher scores on listening comprehension tests when they were provided with subtitles for practice. This finding supports LexiFlow's approach of providing synchronized multimodal input through transcription and translation.

\section{Challenges for Mandarin as Symbolic Language}
\subsection{Character Recognition and Sound Awareness}
Many learners are thrown a curve by the logographic system of writing used in Mandarin, as opposed to alphabetic writing systems, in which written symbols directly represent sounds. According to~\citet{everson2011}, a Western learner typically needs to encounter a Chinese character approximately 17 times before reliably recognizing it, compared to only 8 exposures for words in alphabetic scripts. This added "mental load" is due to the visual complexity of the characters.

Since there is a gap between written and spoken Mandarin, linguists have termed it the `orthographic barrier'~\citep{shen2007}. This barrier hinders learning effectiveness: students may understand spoken vocabulary yet remain unable to connect it to its written form. As a stepping stone to this change, pinyin has become an important tool, but ~\citet{ye2013} noted that 76\% of intermediate learners still experienced problems in accessing character-only materials without the aid of pinyin.
...
Another major difficulty in the literature noted is Mandarin's tonal quality. Wang~\cite{wang1999} has shown that adult learners from non-tonal language backgrounds require explicit training to accurately perceive and produce Mandarin's four tones. They found that beginners' accuracy in recognizing tones was still under 40 percent after 200 hours of classroom learning, without explicit training.

Tone is typically handled in textbooks by isolated examples or in perfectly prepared dialogues. These methods are unnatural ways to speak the language. LexiFlow provides authentic content with pinyin annotations that include tone markers, offering students meaningful opportunities to develop tonal awareness.

\section{Current System Analysis}
The model in which students learn from authentic content is facing great difficulties both for students and teachers. The current process, observed through interviews with Mandarin language teachers at UTM, requires considerable manual effort and yields a limited content library.
...
After content is identified, the real challenges start. The Mandarin audio must be manually transcribed, and instructors must add pinyin annotations and translations, which may take 8-10 hours for a 5-minute video. The volume of transcription work required makes it very difficult to provide a lot of content to students. If students are trying to go through this process alone, there are more obstacles to their understanding because there is no instant feedback.

The resulting materials often lack synchronized timelines that would enable learners to link the spoken and written words together. In addition, the material is often pitched at a level accessible only to more advanced students, leaving intermediate learners without suitable resources.
...
The process starts from the users submitting their actual Mandarin audio or video material to the platform. LexiFlow then automatically segments the audio into sentence-level units using voice activity detection algorithms. Each segment is individually transcribed by ASR, annotated with pinyin, and translated into English by NMT.
...
The three systems already in place all make use of video content to learn a language, but they demonstrate distinct strategies to deal with the same core issue: the provision of engaging authentic video content supported by accurate language. The three platforms all rely on existing or manually created subtitles{as shown in table~{\ref{tab:feature_comparison}}}. This leads to a content bottleneck that may have a negative impact on Mandarin learners because there is not enough content with subtitles, especially when compared to European languages such as French and German.
...
\citet{barrault2023seamless} introduced SeamlessM4T v2, Meta's massively multilingual and multimodal machine translation model, designed to provide high-quality, real-time translation across speech and text in nearly 100 languages.  The v2 release features the UnitY2 architecture, which includes hierarchical character-to-unit upsampling and non-autoregressive decoding.

Importantly for LexiFlow,~\citet{gary2024} showed that even when translations had small faults, showing learners both the source text and machine translations increased comprehension compared to source-only presentation. This result demonstrates that LexiFlow's strategy of offering translations in addition to the original Mandarin content works well.
...
\section{Summary}
This literature review has established several key findings that inform LexiFlow's development. First, it was established that Krashen's Input Hypothesis and Multimedia Learning Theory provide strong theoretical foundations for LexiFlow's approach. After that, Mandarin's symbolic writing system and its unique challenges were discussed. Furthermore, the current language learning applications and their limitations were carefully examined. Finally, this literature review concluded with a discussion of recent advancements in Voice Activity Detection, Automatic Speech Recognition, and Neural Machine Translation and the models which are planned to be used in the system.
% \begin{enumerate}[label= (\alph*)]
...
\begin{enumerate}[label= (\alph*)]
    ...
    \item \textbf{Actual Sprints (Agile):} The development will take place in six sprints (Agile):
          ...
          \begin{enumerate}[label= (\roman*)]
              ...
              \item \textit{Sprint 4 ~ Interactive Learning Module:} This sprint will deliver the interactive learing module, implementing features such as word search, bookmarking segenents, and reporting incorrect translations.
              ...
              \item \textit{Sprint 6 ~ Deployment Sprint:} This sprint will involve deploying the system to live servers and tesing the live deployment.
          \end{enumerate}
...
\begin{enumerate}[label= (\alph*)]
    \item \textbf{React 18:} A JavaScript library for creating user interfaces. React 18 was chosen for its component-based architecture and concurrent rendering capabilities. It will be used to create the frontend of the web application.
    \item \textbf{Node.js:} a JavaScript runtime based on Chrome's V8 engine, chosen for its non-blocking, event-driven architecture, which makes it suitable for developing scalable network applications. It will be used for server-side logic and user request management. Furthermore, it will act as the control center of our backend services, handling requests from the frontend and coordinating with the database and python microservices.
...
LexiFlow is developed in an automated way, making updates a smooth process. The developers push changes into the code into version control (Git), as shown in Figure ~\ref{fig:ci_cd}. The code is then automatically built and tested on GitHub Actions. Finally, the working application is containerized using Docker to be deployed via Nginx. This automatic pipeline enables the identification of mistakes at an early stage and stable releases.

From the figure it is clear how these steps are interrelated with our planning and monitoring that ensures development is efficient and reliable. This aids in ensuring we are able to continue our hybrid agile approach with regular updates without affecting the stability of the system.
...
\begin{enumerate}[label= (\alph*)]
    ...
    \item \textbf{Version Control:} Git to manage code changes and track version history.
...
This chapter guides through the approach that was taken by LexiFlow with special emphasis on the hybrid Agile framework that combines both iterative development with structured phases. That way, the project could remain flexible enough to adapt to changing needs while still maintaining structured milestone checks. The technology stack has also been discussed in detail with the rationale of each decision. Lastly, the hardware and software requirements are also outlined, which sets the stage of a successful implementation in the subsequent phases.

\chapter{REQUIREMENT ANALYSIS AND DESIGN}
\section{Introduction}
This chapter provides an overview of the requirement analysis and design phases for the LexiFlow project, which guides the development of a systematically organised language learning system designed for the UTM community. The chapter adheres to a systematic way of identification and analysis of specific requirements of various stakeholders through structured interactions like surveys and interviews.

After the detailed requirement analysis is conducted, the chapter then proceeds to design the system. This involves designing a system architecture that supports seamless interaction between LexiFlow's various components. This involves designing the modular structure of the system and how those different modules communicate with each other, ensuring that the architecture achieves the project's stated goals.

\section{Requirements Gathering and Analysis}

The requirements that need to be considered in the LexiFlow project were carefully studied in the interview session with the stakeholders from UTM Language Academy. This was a key part in articulating the constraints of the current system and the needs that the stakeholders might have on the new system, which is LexiFlow. They pointed out that there is an urgent need for an automatic solution that can process the authentic Mandarin content and present synchronized audio, pinyin and English translations to students.
...
\begin{longtable}{>{\centering\arraybackslash}m{1.2cm} >{\centering\arraybackslash}m{3cm} >{\arraybackslash}m{8cm}}
    ...
    \hline
    \endlastfoot
    NFR-001     & Performance   & The system shall process submitted audio/video content (voice activity detection, transcription, translation, and pinyin annotation) within 10 minutes of processing time per hour of source-content duration. Verified in Chapter 5, Section~\ref{sec:nfr_verification}. \\
    \hline
...
Pipe and Filter architectural pattern is a way of structuring data processing in a series of individual modules, which are called filters, connected via pipes. Every filter takes the data, applies a particular transformation or processing and outputs the transformed data to the next filter through a pipe. The modular approach increases the project's scalability, maintainability, and reusability, as filters are developed, tested, and replaced individually. The separation of concerns also helps extend the system and debug the system in the future.

In LexiFlow's audio processing pipeline, each filter may be uniquely designed with different functionality such as voice activity detection, speech recognition, translation, and pinyin generation, which makes the Pipe and Filter pattern appropriate. This modularity can be used to easily install new processing steps or even to replace existing ones without altering the system architecture.

The architecture of the LexiFlow system is composed of various components as depicted in Figure~\ref{fig:pipe_and_filter}:


\begin{enumerate}[label= (\alph*)]
    \item \textbf{User Interface (UI):} The user interface (UI) is created using React 18, providing an interactive application for users to upload media, view subtitles, and manage vocabulary. It serves as the data source in the Pipe and Filter architecture.
...
\section{Chapter Summary}
Chapter 4 discusses the requirements analysis and system design of the LexiFlow system, which presents an in-depth exploration of the development of a language learning platform. In its initial phase, the chapter systematically identifies the needs of the stakeholders, laying the groundwork for the detailed documentation of these requirements in the Software Requirements Specification (SRS, Appendix~\ref{app:srs}) and Software Design Document (SDD, Appendix~\ref{app:sdd}). It also explains the pipeline architecture approach which is used to build an efficient and scalable system and the user interface design, which is essential for further improving the usability and the system functionality. This systematic approach not only meets the goals of the project, but also provides a solid structural foundation for the successful implementation and deployment of LexiFlow. An automated end-to-end test and User Acceptance Testing was then performed to validate the system, and this validation was documented in a Software Test Document as opposed to a separate, standalone Software Test Document, as described in Chapter 5 and Appendices~\ref{app:test-execution-report} and~\ref{app:uat-report}.


\chapter{Implementation and Testing}\label{chap:implementation}

\section{Introduction}
This chapter delves into the actual development of LexiFlow. It first revisits the Pipe and Filter design pattern introduced in Chapter 4 and demonstrates how it was realized in the actual codebase. It then examines the core learning features: subtitle generation, flashcards, quizzes, and crowdsourced error reporting.
\section{Architecture in Practice: Pipe and Filter}Chapter 4 put forward a Pipe and Filter architectural style for the media-processing subsystem. Each stage of subtitle generation is its own filter: it takes the previous stage's output, applies its transformation, and hands its own output downstream, with no stage aware of anything past its immediate neighbour. Figure~\ref{fig:media_pipeline} shows how this played out. A source video gets downloaded and normalised by FFmpeg, transcribed by a cloud Whisper API, translated by an LLM, then finally annotated with pinyin and HSK levels before it's saved as a subtitle document.
...
Figure~\ref{lst:pipeline} shows the orchestrator that wires these filters together. Every step independently reports its own progress and elapsed time, and if one step fails, none of the other filters need to know why they simply receive no input.
...
\end{figure} Distractor selection for multiple-choice questions combines that same LLM-based plausibility check with two lightweight local heuristics: a pinyin Levenshtein-distance score (catching words that sound alike) and a radical/Unicode similarity score (catching ones that look alike). The three scores get summed, then the top candidates from the same HSK tier win out, so a distractor stays believable without ever sneaking in as an accidental synonym of the right answer.
\section{Core Feature: Crowd-Reporting of Translation Errors (UC11)}
Anyone watching can flag a subtitle line as incorrect, selecting a category: translation, pinyin, transcription, or other (Figure~\ref{fig:ss_crowdreport}). However, a line is only escalated for correction after it clears a deliberately conservative dual threshold (Figure~\ref{lst:flag}): at least three different reporters, \emph{and} at least 25\% of everyone who actually saw that line. So one annoyed viewer can't flag stuff on their own. \begin{figure}[H]
    \centering
...
Figure~\ref{fig:deployment_topology} lays out where each piece of the system actually lives. The frontend is a static build served from \textbf{Vercel}'s edge CDN. The Node.js backend, the FastAPI worker, and PostgreSQL all sit together on one \textbf{Tencent Cloud Lighthouse} VM through Docker Compose, with a \textbf{SaladPool} instance kept around as a rarely-touched failover worker. On every push to \texttt{main}, \textbf{GitHub Actions} builds and pushes a container image to GHCR, then redeploys the VM on its own, so there's no manual deployment step. All AI inference: transcription, translation, quiz grading, segment correction, is delegated to externally hosted language models (Whisper Large v3 Turbo, DeepSeek-V4-Flash, and Llama 3.3 70B), which means the deployment needs no GPU. The complete source code repository is publicly available on GitHub at \url{https://github.com/amer-adam/lexiflow}, and the live application is accessible at \url{https://lexiflow.amerai.top}. \begin{figure}[H]
    \centering
...
The system was validated using two distinct methods. The first was an automated end-to-end Playwright suite which was executed directly against the live test deployment from UC01-04 to UC13-14. Of 30 test cases, 25 passed, 4 failed, and 1 was intentionally skipped. These failures were not due to problems with the test scripts but rather stemmed from two real, reproducible defects: one was the onboarding tour which could not be dismissed and the other was a missing single-page-application fallback route. Further details of the methodology, per test-case results and root cause analysis are provided in Appendix~\ref{app:test-execution-report}. Then came User Acceptance Testing, conducted with four users, representing four user groups identified in Chapter 4: a Mandarin learner who has been having trouble with the language, a self-directed language learner who requires some domain-specific vocabulary, a native speaker reviewer to ensure the correctness of the transcription and translation, and the instructor stakeholder. They returned feedback on the merit of the Tap-to-Look-Up, Vocabulary-Saving, and Quiz-Generation flows. It also brought two honest problems to the fore for later: the lack of accuracy when compared to commercial-grade captioning and the lack of classroom-management features, both of which were not functional requirements in Chapter 4. Full participant profiles, scenarios and acceptance-criteria analysis is found in Appendix~\ref{app:uat-report}.

\subsection{Non-Functional Requirement Verification}\label{sec:nfr_verification}
To verify compliance with NFR-001 (Table~\ref{tab:non_functional_requirements}) that submitted content is processed within 10 minutes per hour of source-content duration  16 real videos ranging from 11 seconds to 55 minutes were submitted to the production media-processing pipeline running on CUDA-accelerated infrastructure, and their end-to-end processing times were recorded. Table~\ref{tab:nfr_performance} lists the results, sorted by content length, together with the ratio of processing time to content duration, the implied processing minutes per hour of content, and whether each run met the target.

\begin{table}[H]
    \centering
    \footnotesize
    \caption{NFR-001 (Performance) verification results, 16 production processing runs}\label{tab:nfr_performance}
    \begin{tabular}{lcccc}
        \toprule
        \textbf{Video} & \textbf{Content} & \textbf{Processing} & \textbf{Min./Hour}  & \textbf{Meets}  \\
                       & \textbf{Length}  & \textbf{Time}       & \textbf{of Content} & \textbf{Target} \\
        \midrule
        V1             & 0:11             & 0:12                & 65.5                & Fail            \\
        V2             & 3:00             & 0:40                & 13.3                & Fail            \\
        V3             & 3:18             & 0:41                & 12.4                & Fail            \\
        V4             & 9:03             & 1:23                & 9.2                 & Pass            \\
        V5             & 9:35             & 1:00                & 6.3                 & Pass            \\
        V6             & 11:24            & 2:27                & 12.9                & Fail            \\
        V7             & 14:25            & 1:59                & 8.3                 & Pass            \\
        V8             & 16:41            & 2:19                & 8.3                 & Pass            \\
        V9             & 18:52            & 6:55                & 22.0                & Fail            \\
        V10            & 20:47            & 2:35                & 7.5                 & Pass            \\
        V11            & 24:10            & 3:01                & 7.5                 & Pass            \\
        V12            & 25:00            & 3:31                & 8.4                 & Pass            \\
        V13            & 43:39            & 5:09                & 7.1                 & Pass            \\
        V14            & 47:26            & 5:26                & 6.9                 & Pass            \\
        V15            & 51:44            & 6:04                & 7.0                 & Pass            \\
        V16            & 55:02            & 7:14                & 7.9                 & Pass            \\
        \bottomrule
    \end{tabular}
    \begin{flushleft}
        \small Target: no more than 10 minutes of processing time per hour of source-content duration. Video identifiers are anonymised run labels from the underlying processing log, sorted by content length.
    \end{flushleft}
\end{table}

Eleven of the sixteen runs (68.8\%) met the target. Content nine minutes or longer met the target in every run but one. The five runs that missed the target were, with one exception, all short clips under 11 minutes: each processing job carries a largely fixed startup cost (model and API connection setup, segmentation initialisation) that is independent of content length, so that overhead is proportionally larger, and the resulting minutes-per-hour ratio correspondingly worse, for short inputs. The one exception, V9 (18 minutes 52 seconds), took considerably longer than its neighbours in the dataset; this is treated as a genuine outlier rather than dismissed, and its cause (for example, a period of elevated load on an externally hosted API) was not isolated in this round of testing. Overall, the pipeline reliably meets its performance target for the long-form authentic content LexiFlow is designed around, while short-clip processing, an edge case outside the platform's primary use case, remains an area for future optimisation, such as amortising or reducing fixed per-job startup overhead.

\section{Chapter Summary}
This chapter has shown how the Pipe and Filter architecture described in Chapter 4 transitioned into a pipeline where each stage can be independently replaced, and explored the four primary learning features , subtitle generation, flashcards, quizzes, and crowdsourced corrections. Each one combines a deterministic local algorithm (FSRS, threshold checks, pinyin/visual similarity) with an LLM only when open-ended judgement is required. Deployment was intentionally kept lightweight; all AI inference was delegated to external APIs. Through automated end-to-end testing and User Acceptance Testing, the system's overall functionality was confirmed, and all identified defects traced back to specific, repeatable root causes rather than to architectural flaws.

\chapter{CONCLUSION}
\section{Introduction}
This chapter reviews LexiFlow's development, its outcomes, and the extent to which the project met its initial objectives. The system has been fully built, tested through automated end-to-end testing and User Acceptance Testing, and deployed on live infrastructure. The focus here is on the results, the key successes, and areas identified for future improvement. Through continuous development and refinement, LexiFlow has the potential to become a reliable and widely adopted language learning platform for universities and beyond.

\section{Achievement of Project Objectives}The LexiFlow project had four main goals, which were specified in Section~\ref{sec:obj} of this paper.  They were all designed to make language learning easier.

\begin{enumerate}[label= (\alph*)]
    \item \textbf{Elicit full Requirements from Stakeholders:} The first goal was to gather a complete set of requirements from stakeholders. This was accomplished by interviewing stakeholders in a round-robin fashion. Through these discussions, a clear picture emerged of the language-learning challenges faced by users and the extent of language barriers within the UTM community. These requirements became functional and non-functional specifications for the project, all documented in the Software Requirements Specification (SRS) in Appendix~\ref{app:srs}. This objective was successfully completed.
    \item \textbf{Design a Web-Based Platform:} The second objective was to design a web-based platform with an appropriate architecture. LexiFlow was designed around a Pipe and Filter architecture. React 18 for the front end enabled the creation of a user-friendly and responsive interface, while Python and Node.js were used on the back end. The entire detailed design and documentation for the system is contained in the Software Design Document (SDD) (see Appendix~\ref{app:sdd}). This objective was also successfully achieved.
    \item \textbf{Develop the Full Platform:} The third objective was to build the full language learning platform. This was achieved by implementing the complete audio processing pipeline — voice activity detection, automatic speech recognition, translation, and pinyin generation, alongside the interactive learning features: bilingual subtitle playback, flashcards, quizzes, and crowdsourced error correction. All features were derived directly from the stakeholder requirements and design specifications documented in Chapters 3 and 4.
    \item \textbf{Evaluate and Test the Functionality:} The last goal was to test the efficacy and improve the language learning process of LexiFlow. This was done via an automated end-to-end Playwright test suite with use cases UC01–UC14, which passed 25 of 30 tests cases, and identified 2 defects, and via User Acceptance Testing with 4 representative users from within the UTM community. The outcome of the tests is recorded in Test Execution Report (Appendix~\ref{app:test-execution-report}) and UAT Report (Appendix~\ref{app:uat-report}). This objective is deemed to be achieved.
\end{enumerate}
\section{Service Enhancement and Practical Value}LexiFlow does more than meet the goals it set out to achieve. It delivers on two of the three dimensions commonly used to evaluate whether a system is genuinely useful: service improvement and quality-of-life improvement for its users. Commercial value has not yet been assessed, as no pricing model or market testing has been conducted. In terms of service quality, the stakeholder interview (Appendix~\ref{app:stakeholder-interview-questions}) revealed that each 3-minute clip of authentic Mandarin content required 10 to 30 minutes of manual work to transcribe, translate, and annotate with pinyin. This has been replaced by a pipeline that learners and teachers can trigger at will, and a crowd-correction feature allows for ongoing improvement without requiring that work to be repeated. This represents a significant change from the situation described by stakeholders, where very little authentic content was used due to the high cost of preparation. Quality-of-life improvements were independently confirmed during UAT (Appendix~\ref{app:uat-report}). One struggling student reported that this platform would have helped them achieve a better result on their actual exam. A native Mandarin speaker described using the platform daily to reduce real-world miscommunication in workplace settings , problems they had never encountered in classroom activities or through generic applications and textbooks. These are not assumed benefits but outcomes that users explicitly reported, reflecting genuine improvements in their learning and spoken communication confidence.
\section{Suggestions for Future Improvement}Building on findings from automated testing and UAT, the following improvements are recommended for future versions of LexiFlow. First are the two known defects identified during test execution. The onboarding tour that cannot be dismissed (LF-BUG-01) and a missing SPA fallback route, which results in a raw 404 error when navigating directly to a URL (LF-BUG-02). Both need to be addressed before any production rollout: one prevents core navigation for first-time visitors, while the other affects returning users who navigate directly via URL. Secondly, there is the accuracy gap that the native-speaker UAT participant pointed out between the faster-whisper in LexiFlow's self-hosted pipeline and commercial-grade auto-captioning on YouTube. This accuracy gap should be narrowed in future development. The crowd-reporting and AI-assisted correction feature described in Chapter 5 is well-suited for addressing this gap over time; measuring its long-term effectiveness in a subsequent evaluation round would be worthwhile. Third, and most significantly, is the need for a classroom-management module , a module that allows lecturers to assign videos or quizzes to a class and monitor each student's progress through a dedicated dashboard. It was a significant unmet need expressed by instructor stakeholders during UAT and was not part of the initial functional requirements, making it a clear priority for the next development phase. Beyond the aforementioned evidence-based priorities is the decision to replace the large multilingual model with a lighter Mandarin-specific local model, reducing processing time and server VRAM and thus reducing operating costs. Further reductions in runtime and resource consumption could be achieved by adopting leaner inference runtimes such as ONNX or TensorRT. All these changes combined would make LexiFlow more precise, more convenient for institutions and more economical to operate.

\bibliographystyle{utmthesis-numbering} % Official UTM numbered (IEEE-style) bibliography style
...
% NOTE: This appendix was left with no content (no screenshot/body was ever added) -
% commented back out to avoid submitting a blank appendix chapter. Add a Jira
% screenshot (e.g. board/burndown) and uncomment if you want to include it.
