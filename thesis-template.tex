...
\section{Introduction}
This chapter discusses the technical implementation of the LexiFlow system. The development is divided into modular subsystems: User Management, Media Processing and AI, Interactive Learning, Assessment, Supplementary Services, DevOps and Deployment, Database Architecture, and Security. Each module is designed to fulfill the specific functional requirements outlined in the system design phase.
...
At the current stage of implementation, user profile information (display name, email, and picture) is sourced directly from the verified Auth0 identity token rather than from a dedicated, editable profile record in the database. A dedicated backend capability for storing and editing custom preferences (e.g., target learning language or default study parameters) has been scaffolded but is not yet populated with functionality; this is identified as outstanding work for a subsequent development phase.
...
\begin{itemize}
    \item \textbf{Data Handled:} User profiles, vocabulary lists, saved words, flashcard decks, FSRS spaced repetition algorithms states, and user quiz scores.
