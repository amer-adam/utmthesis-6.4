...
This module represents the core pipeline of LexiFlow, managed by the FastAPI subsystem. It handles the ingestion of media and the AI-driven generation of linguistic data.
...
The production environment of LexiFlow is split across four distinct hosting environments, each chosen to match the resource profile of the component it serves: a static-asset CDN for the frontend, a low-cost always-on cloud VM for the relational backend, a managed cloud database for unstructured/legacy data, and a GPU-capable host for the AI media engine. Figure~\ref{fig:deployment_topology} shows how these environments are connected, including the automated CI/CD path and the GPU failover path described in the following subsections.
...
\begin{itemize}
    ...
    \item \textbf{Queue System:} The asynchronous nature of media processing (downloading, audio extraction, transcription, and translation) requires a robust task queue. MongoDB is utilised to track the real-time status of these asynchronous jobs, recording the progress states of the backend worker nodes and allowing the system to update the frontend on video processing progress without locking or overloading the primary PostgreSQL database.
