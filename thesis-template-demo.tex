...
LexiFlow addresses these challenges through a web platform that integrates sentence segmentation and machine translation models. The system receives audio, segments it into sentences, and processes each one separately. This will help to increase accuracy and reduce the computing resources required. The system then maintains a repository of previously processed material to avoid reprocessing and to surface content tailored to students' interests and abilities.
\section{Problem Background}

The lecturer of language at UTM has noticed a problem with the development of language among the students at UTM. Many students can master grammar rules and vocabulary for written exams but have trouble in real Mandarin conversations. This disparity between students' academic performance and their communication skills is a hindrance to learning. Authentic content exposes students to vocabulary, natural speech patterns, and cultural context that textbooks cannot provide, especially when the content aligns with students' own interests, making the language more engaging.
...
\begin{enumerate}[label= (\alph*)]
    ...
    \item \textbf{Evaluate and Test the Functionality:} The final objective was to validate how well LexiFlow performs and enhances the language learning experience. This was carried out through an automated end-to-end Playwright test suite covering use cases UC01 through UC14, yielding 25 passes out of 30 test cases with two identified defects, and through User Acceptance Testing with four representative users from the UTM community. The results are documented in the Test Execution Report (Appendix~\ref{app:test-execution-report}) and UAT Report (Appendix~\ref{app:uat-report}). This objective is considered complete.
\end{enumerate}
